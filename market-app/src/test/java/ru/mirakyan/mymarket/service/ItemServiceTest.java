...
class ItemServiceTest {
    ...
    void testGetItems_WithoutSearchAndSort() {
        when(itemCacheService.getAllItemsFromCache()).thenReturn(Flux.empty());
        when(itemRepository.findAll()).thenReturn(Flux.just(item1, item2, item3));
        when(itemCacheService.saveAllItemsToCache(anyList())).thenReturn(Mono.just(3L));
    ...
    void testGetItemById() {
        when(itemCacheService.getItemFromCache(1L)).thenReturn(Mono.empty());
        when(itemRepository.findById(1L)).thenReturn(Mono.just(item1));
        when(itemCacheService.saveItemToCache(any(Item.class))).thenReturn(Mono.just(item1));
    ...
    void testGetItemById_NotFound() {
        when(itemCacheService.getItemFromCache(999L)).thenReturn(Mono.empty());
